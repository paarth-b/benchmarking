...
To address this computational bottleneck, we systematically evaluate efficient protein language models (Lobster, Ankh, ProtMamba) as alternatives to ProtT5-XL. Despite its smaller size, Lobster-24M showcased an optimal balance between accuracy and efficiency, which led to its selection as our foundation model. Firstly, we use a BiLSTM architecture to reduce time complexity as compared to the attention mechanism implemented in the transformer-based TM-Vec. Second, we develop a lightweight neural architecture (1.3M parameters) that embeds sequences into a latent space, where cosine similarity between vectors correlates with TM-score. The student model is trained to predict embeddings directly from raw amino acid sequences, whose cosine similarity can compute TM-scores, achieving comparable accuracy to TM-Vec with orders of magnitude fewer computational resources.

% TODO: add tm-vec 2.0
% In modern LLMs, it is common to leverage model distillation to construct a smaller model that approximates the functionality of the original model (CITE mistral, deepseek, etc). By doing so, these smaller models are faster to run, since they have fewer operations required to generate predictions. 


% To address this computational bottleneck, we use a BiLSTM architecture to reduce time complexity as compared to the previous attention mechanism implemented in the transformer-based TM-Vec. Furthermore, we develop a lightweight neural architecture (1.3M parameters) that learns to embed sequences into a latent space, where cosine similarity between vectors correlates with TM-score, achieving comparable accuracy to TM-Vec with orders of magnitude fewer computational resources.
...
% TODO: Add info from MLSB Appendix A/B here.
 ...
 \begin{figure}
 \centering
 \includegraphics[width=\linewidth]{illustration.png} \caption{\label{fig:illustration}{For each protein pair $(X,Y)$, the TM-Vec teacher generates a TM-score. The student model encodes both sequences using a compact BiLSTM architecture and outputs a predicted TM-score. Training minimizes a composite distillation objective that aligns the student’s predictions with the teacher’s outputs, enabling the student to approximate the structural topology learned by TM-Vec without using protein structures.}}
 \end{figure}


\subsection{TM-Vec Student: Knowledge Distillation Architecture}

The TM-Vec Student model eliminates the dependency on protein language models entirely through knowledge distillation. Operating directly on raw amino acid sequences, the model comprises 1.3 million parameters, which is a significant reduction of over 2,000× compared to the previous ProtT5-XL based architecture. The architecture employs three key components: a BiLSTM-based sequence encoder, a feature fusion module, and a calibrated TM-score predictor.

\subsubsection{Sequence Encoder}
The architecture utilizes a BiLSTM backbone \cite{hochreiter1997long,graves2005framewise} prioritizing linear space complexity over the quadratic costs of transformer-based alternatives. \cite{abduljabbar2021development}. The encoder maps each amino acid to a 128-dimensional embedding, followed by a 2-layer BiLSTM with 256 hidden units per direction. An attention pooling mechanism aggregates the bidirectional hidden states into a single 256-dimensional sequence representation. This vector then passes through a projection head (256d $\rightarrow$ 512d) to produce the final embedding $z$, maintaining compatibility with the teacher model's embedding space.

\subsubsection{Feature Fusion and TM-Score Prediction}
Given sequence embeddings $z_a$ and $z_b$, we construct a combined representation: $[z_a, z_b, z_a \odot z_b, |z_a - z_b|]$, where $\odot$ denotes element-wise multiplication. This 2048-dimensional fused representation captures direct similarities, interaction patterns, and structural differences. A 5-layer fully-connected network with progressive dimensionality reduction (2048d $\rightarrow$ 512d $\rightarrow$ 256d $\rightarrow$ 128d $\rightarrow$ 64d $\rightarrow$ 1) processes this representation to predict the final TM-score. The TM-score predictor uses ReLU activations and dropout regularization at each stage to prevent overfitting. The network incorporates temperature scaling and bias correction parameters to improve calibration, which is particularly important for reliable similarity thresholds in remote homology detection.

\subsubsection{Training Objective}
The student model is trained using a composite loss function designed to balance regression accuracy with classification performance across biologically meaningful similarity ranges. Following established structural biology thresholds \cite{zhang2005tm,xu2010significant}, we partition TM-scores into three buckets: (1) structurally unrelated proteins (TM-score $< 0.17$), where similarity is likely due to random chance; (2) the middle zone ($0.17 \leq$ TM-score $< 0.6$), and (3) same-fold proteins (TM-score $\geq 0.6$). The loss function combines:
\begin{equation}
\mathcal{L}_{\text{total}} = \mathcal{L}_{\text{MSE}} + \alpha \cdot \mathcal{L}_{\text{balanced}} + \beta \cdot \mathcal{L}_{\text{focal}} + \gamma \cdot \mathcal{L}_{\text{cal}}
\end{equation}
...
$$\mathcal{L}_{\text{balanced}} = \frac{1}{N} \sum_{i=1}^{N} w(y_i) \cdot (y_i - \hat{y}_i)^2$$

$$\mathcal{L}_{\text{focal}} = \frac{1}{N} \sum_{i=1}^{N} |y_i - \hat{y}_i| \cdot \underbrace{(y_i - \hat{y}_i)^2}_{\text{MSE}}$$

$$\mathcal{L}_{\text{cal}} = \left( \underbrace{\left( \frac{1}{N} \sum_{i=1}^{N} \hat{y}_i \right)}_{\text{Batch Mean Pred}} - \underbrace{\left( \frac{1}{N} \sum_{i=1}^{N} y_i \right)}_{\text{Batch Mean True}} \right)^2$$

Here, $N$ is the number of protein pairs, $i$ is the index of a specific sample in the batch ($i = 1, \dots, N$), $y_i$ is true TM-score for sample $i$ (teacher output), $\hat{y}_i$ is the predicted TM-score for sample $i$ (student prediction), $y$: The vector of all true scores in the batch, and $\hat{y}$ is the vector of all predicted scores in the batch. $w(y_i)$ is the inverse frequency weight, such that $w$ is high for rare samples.

\subsubsection{Loss Function}

We use $\mathcal{L}_{\text{MSE}}$ to represent the standard mean squared error for regression, $\mathcal{L}_{\text{balanced}}$ re-weights errors within each bucket to counteract the non-uniform data distribution (ensuring the under-represented high-similarity samples receive adequate representation), $\mathcal{L}_{\text{focal}}$ applies focal weighting to emphasize harder-to-classify examples, and $\mathcal{L}_{\text{cal}} = (\overline{\hat{y}} - \overline{y})^2$ penalizes systematic prediction bias which encourages the student model to match the global distribution of
teacher-predicted TM-scores.  This helps stabilize fold-level thresholds (e.g., $\text{TM} > 0.5$) and reduces systematic over- or under-prediction across similarity regimes. Hyperparameters $\alpha, \beta, \gamma$ balance these components based on validation performance. 
...
The TM-Vec Student model was trained on a dataset of three million protein pairs derived from the ProteinNet database \cite{alquraishi2019proteinnet}, balanced across the three TM-score ranges, using a train/val/test split of 64\%, 16\% and 20\%, respectively. The ground truth TM-score for training the student model was obtained using the TM-Vec model. The student model was trained for 25 epochs on a system with 1x NVIDIA A100 GPU for approximately 7 hours (with a batch size of 64 and AdamW optimizer using learning rate 1e-3, weight decay 0.01).

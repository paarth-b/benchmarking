#!/usr/bin/env python
"""
TM-Vec Student: Generate pairwise TM-score predictions using cosine similarity.

This script uses the cosine-only student model which predicts TM-scores directly
from the cosine similarity of sequence embeddings (instead of MLP predictor head).
...
3. Score every unique pair (i < j) using cosine similarity and
...
Example:
    python -m src.benchmarks.cosine_tmvec_student \
        ...
        --checkpoint binaries/cosine_tmvec_student.pt \
        --output results/cosine_tmvec_student_similarities.csv
"""
...
def load_student_model(checkpoint_path: Path, device: torch.device) -> StudentModel:
    ...
    print(f"Loading Cosine TM-Vec Student checkpoint: {checkpoint_path}")
    ...
    print(f"Cosine student model loaded ({total_params:,} parameters)")
    return model
...
def predict_pairwise_tm_scores(
    ...
    """Score every unique pair using cosine similarity and stream to CSV."""
    ...
    print(f"Scoring {total_pairs:,} sequence pairs using cosine similarity → {output_path}")
        ...
        with torch.no_grad():
            ...
            for i in range(num_sequences - 1):
                ...
                while j < num_sequences:
                    ...
                    preds = F.cosine_similarity(batch_a, emb_j, dim=1)

                    # Clamp to [0, 1] range to match TM-score range
                    preds = torch.clamp(preds, 0.0, 1.0).detach().cpu()
    ...
    print("Pairwise TM-score statistics (cosine similarity):")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=Path("/scratch/akeluska/benchmarking/binaries/cosine_tmvec_student.pt"),
        help="Path to Cosine TM-Vec Student checkpoint (.pt)",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    print("=" * 80)
    print("Cosine TM-Vec Student TM-score Prediction (Cosine Similarity)")
